...
$(document).ready(function () {
    ...
    if ($("#step").val() == "1") {
        ...
        if (
            localStorage.getItem("comentarios_internos") != "" &&
            localStorage.getItem("comentarios_internos") != undefined
        ) {
            console.log("INGRESO TCI");
            sceditor
                .instance(tci)
                .val(localStorage.getItem("comentarios_internos"));
        }
        if (
            localStorage.getItem("comentarios_escuelas") != "" &&
            localStorage.getItem("comentarios_escuelas") != undefined
        ) {
            console.log("INGRESO TEC");
            sceditor
                .instance(tce)
                .val(localStorage.getItem("comentarios_escuelas"));
        }
        if (
            localStorage.getItem("info_curso") != "" &&
            localStorage.getItem("info_curso") != undefined
        ) {
            console.log("INGRESO TIC");
            sceditor.instance(tic).val(localStorage.getItem("info_curso"));
        }
        if (
            localStorage.getItem("info_adicional") != "" &&
            localStorage.getItem("info_adicional") != undefined
        ) {
...
function setSemanas() {
    ...
    if (iniObj != null && finObj != null && nocObj != null) {
        ...
        if (iniValue != null && finValue != null) {
            MD_Y = iniValue.substring(6, 10);
            MD_D = iniValue.substring(0, 2);
            MD_M = iniValue.substring(3, 5);
            var date2 = new Date(iniObj.value);
            MD_Y = finValue.substring(6, 10);
            MD_D = finValue.substring(0, 2);
            MD_M = finValue.substring(3, 5);
...
function validateIsNotNull(value, maxsize, type) {
    if (value.length != 0) {
        if (value.length > maxsize) {
            return {
                res: false,
                message: "**Campo " + type + " demasiado extenso",
            };
        } else {
            return true;
        }
...
function clearStep1() {
    confirmacionEliminar(
        "¿Desea limpiar los campos del formulario?",
        function (response) {
            if (response) {
                $("select").val("").change();
                $("#fecha").val("");
