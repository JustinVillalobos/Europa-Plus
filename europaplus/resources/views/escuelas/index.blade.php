        ...
        <div class="" style="padding-left:5px;">
            <ol class="breadcrumb">
                <li class="breadcrumb-item active" aria-current="page">
                    <h5><i class="fa fa-book" aria-hidden="true"></i>Escuelas</h5>
                </li>
            </ol>
        </div>
    </div>
...
<div class="row" style="margin-top:15px;">
    ...
    <div class="col-sm-6 d-flex justify-content-end" style="margin-bottom:20px;height:35px;padding:0px 20px 0px 20px;">
        <form action='{{route("escuelas.busqueda")}}' method="GET" class="d-flex"  style="margin: 0px;">
            @method("GET")
            @csrf
    ...
    <div class="col-sm-12" style="padding:10px 20px 0px 20px;">
        <table class="table table-bordered" style="margin-bottom:3px;">
            ...
            <tbody>
                @foreach($escuelas as $nivel)
                    <tr>
                    <td>{{$nivel->esc_nombre_corto}}</td>
                        <td>{{$nivel->esc_nombre}}</td>
                        <td>{{$nivel->opc_descr}}</td>
                        <td>{{$nivel->pais_descr}}</td>
                        <td style="width:125px;" class="d-flex justify-content-center">
                            
                            <form action='{{route("escuelas.edit", [$nivel])}}' method="post" >
                                @method("get")
                                @csrf
                            ...
                            <form action='{{route("escuelas.destroy", [$nivel])}}' method="post" onsubmit="return validate(event,this,{{$nivel->esc_id}})">
                                @method("delete")
                                @csrf
