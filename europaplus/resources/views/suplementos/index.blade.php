        ...
        <div class="" style="padding-left:5px;">
            <ol class="breadcrumb">
                <li class="breadcrumb-item active" aria-current="page">
                    <h5><i class="fa fa-book" aria-hidden="true"></i>Suplementos</h5>
                </li>
            </ol>
        </div>
    </div>
...
<div class="row" style="margin-top:15px;">
    <div class="col-sm-6" style="padding-left:15px;">
        <a href='{{route("suplementos.create")}}' class="btn btn-primary" style="margin-left:5px;height:35px;">
                <i class="fa fa-plus"></i> Agregar Suplemento
        </a>
    </div>
    <div class="col-sm-6 d-flex justify-content-end" style="margin-bottom:20px;height:35px;padding:0px 20px 0px 20px;">
        <form action='{{route("suplementos.busqueda")}}' method="GET" class="d-flex"  style="margin: 0px;">
            @method("GET")
            @csrf
        ...
        <a href='{{route("suplementos.index")}}' class="btn btn-primary" style="margin-left:5px">
    ...
    <div class="col-sm-12" style="padding:10px 20px 0px 20px;">
        <table class="table table-bordered" style="margin-bottom:3px;">
            ...
            <tbody>
                @foreach($suplementos as $nivel)
                    <tr>
                    <td>{{$nivel->sup_nombre}}</td>
                        <td>{{$nivel->sup_descr}}</td>
                        <td style="width:125px;" class="d-flex justify-content-center">
                            
                            <form action='{{route("suplementos.edit", [$nivel])}}' method="post" >
                                @method("get")
                                @csrf
                            ...
                            <form action='{{route("suplementos.destroy", [$nivel])}}' method="post" onsubmit="return validate(event,this,{{$nivel->sup_id}})">
                                @method("delete")
                                @csrf
    ...
    <div class="col-sm-12" style="padding:0px 20px 10px 20px;">
        {{ $suplementos->links('vendor.pagination.bootstrap-4') }}
    </div>
</div>
<?php $route2 = route("suplementos.index");?>
<input type="hidden" value="{{$route2}}" id="route" />
<script src="{{ URL::asset('js/suplementos/list.js'); }}"></script>       
@stop